...
nums.each do |num|
   puts "==> #{num}"
   name = "%03d" % num
   blob = read_blob( "./martians/ethscribe/martian#{name}.png" )
   ...
   inscriptionNumber =  if File.exist?( "./martians/hashcheck/#{name}.json" )
   ...
   items << { id: hash,
              name: "##{num}",
              image: "https://github.com/0xCompute/punks12px/raw/master/martians/ethscribe/martian#{name}.png",
